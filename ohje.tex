...
\bigskip\noindent
Tiedostoon \verb|/etc/default/solr.in.sh| pitää lisätä rivi
...
baseDir        & Indeksoitavat hakemistot, esimerkiksi \\
               & \texttt{\$\{user.home\}/Asiakirjat:/usr/local/data} \\
               & Erottimena on \texttt{\$\{path.separator\}}
\end{tabularx}
...
\bigskip
\verb=src/main/resources/indexer.sh= ei toimi (ainakaan minulla)
Solr:n versiossa 9.9.0, vaan se yrittää joskus indeksoida
tekstitiedostoja, joitten MIME-tyyppi on \verb=text/plain= tai
\verb=text/x-tex= ikään kuin ne olisivat jotain Microsoftin
formaattia.

Mutta ei hätää, olen tehnyt ohjelman

\verb=src/main/resources/indexer2.sh=

joka käyttää samaa asetustiedostoa kuin \verb=indexer.sh= ja toimii
ainakin minulla. En väitä, että se toimii sinulla.
